...
rem 在工作目錄建立 PowerShell 腳本，而非系統臨時目錄
echo $fileTypes = @('.txt','.xml','.html','.htm','.css','.js','.java','.cs','.cpp','.h','.c','.php','.py','.bat','.cmd','.ps1','.json','.config','.yml','.yaml','.md','.sql') > "convert_eol.ps1"
...
echo     # 使用 StreamReader 檢測並保留原始編碼 >> "convert_eol.ps1"
echo     $sr = New-Object System.IO.StreamReader($file.FullName, $true) >> "convert_eol.ps1"
echo     $content = $sr.ReadToEnd() >> "convert_eol.ps1"
echo     $encoding = $sr.CurrentEncoding >> "convert_eol.ps1"
echo     $sr.Close() >> "convert_eol.ps1"
echo     if ($content -match "[^\r]\n") { >> "convert_eol.ps1"
echo       $newContent = $content -replace "([^\r])\n", "`$1`r`n" >> "convert_eol.ps1"
echo       # 使用相同的編碼寫回檔案 >> "convert_eol.ps1"
echo       $sw = New-Object System.IO.StreamWriter($file.FullName, $false, $encoding) >> "convert_eol.ps1"
echo       $sw.Write($newContent) >> "convert_eol.ps1"
echo       $sw.Close() >> "convert_eol.ps1"
...
echo       Write-Host "檔案 $($file.Name) 已經是 CRLF 格式，不需轉換" -ForegroundColor Green >> "convert_eol.ps1"
echo     } >> "convert_eol.ps1"
